...
class CharDecoder(nn.Module):
    def __init__(self, hidden_size, char_embedding_size=50, target_vocab=None):
        ...
        self.char_output_projection = nn.Linear(self.hidden_size, self.vocab_len, bias=True)
        self.decoderCharEmb = nn.Embedding(self.vocab_len, self.char_embedding_size, padding_idx=0)
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        input = torch.tensor(start_char, dtype=torch.long, device=device).t()
        decodedWords = []
        for step in range(max_length):
            score, dec_hidden = self.forward(input, dec_hidden)
            input = score.argmax(dim=2)
            decodedWords.append(input)
