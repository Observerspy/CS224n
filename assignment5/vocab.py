...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [
            [[self.start_of_word] + [self.char2id[char] for char in word] +
            [self.end_of_word] for word in sent] for sent in sents
        ]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_padded = pad_sents_char(word_ids, self['<pad>'])
        sents_var = torch.tensor(sents_padded, dtype=torch.long, device=device)
        return sents_var.permute(1,0,2).to(device)
